...
def test_create_url():
    ...
    assert (wpc_map_scraper._build_page_url(dt, maptype)) == expected
...
def test_make_iso_date():
    ...
    assert (wpc_map_scraper._make_iso_date(date_str, time_str)) == expected
...
def test_build_no_time_series():
    ...
    stop = '2016-12-31'
    times = []
    assert (wpc_map_scraper._make_time_series(start, stop, times)) == []
...
def test_build_time_series():
    ...
    stop = '2017-01-01'
    times = [0, 6, 18]
    assert (len(wpc_map_scraper._make_time_series(start, stop, times))) == 6
...
def test_short_build_time_series():
    ...
    stop = '2017-01-01'
    times = ['06', '18']
    actual = wpc_map_scraper._make_time_series(start, stop, times)
    start_dt = wpc_map_scraper._make_iso_date(start, times[0])
    stop_dt = wpc_map_scraper._make_iso_date(stop, times[1])
    assert(len(actual)) == 2
...
def test_page_url():
    ...
    date_str = '2017-06-15'
    time_str = '21'
    date_time = wpc_map_scraper._make_iso_date(date_str, time_str=time_str)
    assert (wpc_map_scraper._build_page_url(date_time, 'usfntsfc')) == expected
...
def test_map_path():
    ...
    dt = datetime(year=2017, month=6, day=1, hour=12, tzinfo=timezone.utc)
    maptype = 'namussfc'
    assert (wpc_map_scraper._get_map_path('~/Downloads/wx_maps', dt, maptype)) == expected
...
def test_build_image_url():
    ...
    dt = datetime(year=2017, month=6, day=1, hour=12, tzinfo=timezone.utc)
    maptype = 'namussfc'
    page_url = wpc_map_scraper._build_page_url(dt, maptype)
    assert (wpc_map_scraper._build_image_url(page_url)) == expected
